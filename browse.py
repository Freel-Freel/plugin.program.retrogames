...
try:
	if addon.getSetting("RD")<>"":
		RDir = os.path.join( addon.getSetting("RD"),"" )
		if os.path.exists(RDir) == False: os.makedirs(RDir)
	else: RDir = os.path.join( addon.getAddonInfo('path'),"" )
except:
	RDir = os.path.join( addon.getAddonInfo('path'),"" )
...
def dir(path, cover=None):
	hide=[".ccd", ".conf", ".img", ".sub", ".state", ".iso", ".srm", ".bin", ".png", ".jpg", ".7z", ".CCD", ".CONF", ".IMG", ".SUB", ".STATE", ".ISO", ".SRM", ".BIN", ".PNG", ".JPG", ".7Z"]
	ld=os.listdir(path)
	for i in ld:
